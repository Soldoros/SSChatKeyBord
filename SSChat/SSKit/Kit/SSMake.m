...
void makeUserToken(NSString *token){
    NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
    [user setValue:token forKey:USER_Token];
}

void makeUserHeaderImg(NSString *headerImg){
    NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
    [user setValue:headerImg forKey:USER_HeaderImg];
}

void makeUserRefreshToken(NSString *refreshToken){
    NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
    [user setValue:refreshToken forKey:USER_RefreshToken];
}
...
//app主题设置  原始1  经典黑2  秋意黄3
void makeTitleType(NSInteger type){
   NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
    [user setInteger:type forKey:USER_TitleType];
    
    //原始
       if(type == 1){
           [user setValue:@"#A92D2D" forKey:USER_TitleColor];
           [user setValue:@"#FFFFFF" forKey:USER_NavColor];
           [user setValue:@"#111111" forKey:USER_NavTitColor];
       }
       //经典黑
       if(type == 2){
           [user setValue:@"#111111" forKey:USER_TitleColor];
           [user setValue:@"#111111" forKey:USER_NavColor];
           [user setValue:@"#FFFFFF" forKey:USER_NavTitColor];
       }
       //秋意黄
       if(type == 3){
           [user setValue:@"#DDA76B" forKey:USER_TitleColor];
           [user setValue:@"#DDA76B" forKey:USER_NavColor];
           [user setValue:@"#FFFFFF" forKey:USER_NavTitColor];
       }
...
UIColor *getTitleColor(void){
    NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
    NSString *string = [user valueForKey:USER_TitleColor];
    return makeColorHex(string);
}
UIColor *getNavColor(void){
    NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
    NSString *string = [user valueForKey:USER_NavColor];
    return makeColorHex(string);
}
UIColor *getNavTitColor(void){
    NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
    NSString *string = [user valueForKey:USER_NavTitColor];
    return makeColorHex(string);
}

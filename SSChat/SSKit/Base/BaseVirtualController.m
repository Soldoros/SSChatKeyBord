...
-(void)setNavgationBarLine:(UIColor *)color{
    ...
    self.navLine.backgroundColor = makeColorHex(@"#E2E2E2");
}
...
-(void)setLeftImgButton{
    ...
    NSString *headerImg = [[user valueForKey:USER_HeaderImg] imageString];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(mineMessageChange) name:NotiMineHeaderChange object:nil];

}

-(void)mineMessageChange{
    NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
    NSString *headerImg = [[user valueForKey:USER_HeaderImg] imageString];
    [self.leftBtn1 setImageWithURL:[NSURL URLWithString:headerImg] forState:UIControlStateNormal placeholder:[UIImage imageNamed:@"headerImg"]];
}
